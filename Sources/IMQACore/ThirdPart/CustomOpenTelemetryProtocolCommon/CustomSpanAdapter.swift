...
public struct CustomSpanAdapter {
  public static func toProtoResourceSpans(spanDataList: [SpanData]) -> [Opentelemetry_Proto_Trace_V1_ResourceSpans] {
    ...
    resourceAndScopeMap.forEach { resMap in
      ...
      resMap.value.forEach { instScope in
        ...
        protoInst.scope = CustomCommonAdapter.toProtoInstrumentationScope(instrumentationScopeInfo: instScope.key)
      ...
      resourceSpan.resource = CustomResourceAdapter.toProtoResource(resource: resMap.key)
  ...
  public static func toProtoSpan(spanData: SpanData) -> Opentelemetry_Proto_Trace_V1_Span {
    ...
    protoSpan.traceID = CustomTraceProtoUtils.toProtoTraceId(traceId: spanData.traceId)
    protoSpan.spanID = CustomTraceProtoUtils.toProtoSpanId(spanId: spanData.spanId)
    if let parentId = spanData.parentSpanId {
      protoSpan.parentSpanID = CustomTraceProtoUtils.toProtoSpanId(spanId: parentId)
    }
    ...
    spanData.attributes.forEach {
      protoSpan.attributes.append(CustomCommonAdapter.toProtoAttribute(key: $0.key, attributeValue: $0.value))
    }
  ...
  public static func toProtoSpanEvent(event: SpanData.Event) -> Opentelemetry_Proto_Trace_V1_Span.Event {
    ...
    event.attributes.forEach {
      protoEvent.attributes.append(CustomCommonAdapter.toProtoAttribute(key: $0.key, attributeValue: $0.value))
    }
    return protoEvent
  }
  ...
  public static func toProtoSpanLink(link: SpanData.Link) -> Opentelemetry_Proto_Trace_V1_Span.Link {
    ...
    protoLink.traceID = CustomTraceProtoUtils.toProtoTraceId(traceId: link.context.traceId)
    protoLink.spanID = CustomTraceProtoUtils.toProtoSpanId(spanId: link.context.spanId)
    link.attributes.forEach {
      protoLink.attributes.append(CustomCommonAdapter.toProtoAttribute(key: $0.key, attributeValue: $0.value))
    }
    return protoLink
  }
