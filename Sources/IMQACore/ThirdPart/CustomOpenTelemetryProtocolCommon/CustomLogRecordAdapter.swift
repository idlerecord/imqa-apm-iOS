...
public class CustomLogRecordAdapter {
  public static func toProtoResourceRecordLog(logRecordList: [ReadableLogRecord]) -> [Opentelemetry_Proto_Logs_V1_ResourceLogs] {
    ...
    resourceAndScopeMap.forEach { resMap in
      ...
      resMap.value.forEach { scopeInfo, logRecords in
        ...
        protoScopeLogs.scope = CustomCommonAdapter.toProtoInstrumentationScope(instrumentationScopeInfo: scopeInfo)
      ...
      resourceLog.resource = CustomResourceAdapter.toProtoResource(resource: resMap.key)
  ...
  static func toProtoLogRecord(logRecord: ReadableLogRecord) -> Opentelemetry_Proto_Logs_V1_LogRecord {
    ...
    if let body = logRecord.body {
      protoLogRecord.body = CustomCommonAdapter.toProtoAnyValue(attributeValue: body)
    }
    ...
    if let context = logRecord.spanContext {
      protoLogRecord.spanID = CustomTraceProtoUtils.toProtoSpanId(spanId: context.spanId)
      protoLogRecord.traceID = CustomTraceProtoUtils.toProtoTraceId(traceId: context.traceId)
    ...
    logRecord.attributes.forEach { key, value in
      protoAttributes.append(CustomCommonAdapter.toProtoAttribute(key: key, attributeValue: value))
    }
