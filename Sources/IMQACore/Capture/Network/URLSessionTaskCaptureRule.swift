//
//  URLSessionTaskCaptureRule.swift
//  Imqa-sdk-ios
//
...
class URLSessionTaskCaptureRule {
    ...
    init(rule: NetworkPayloadCaptureRule) {
        ...
        do {
            ...
            IMQA.logger.error("Error trying to create regex \"\(rule.urlRegex)\" for rule \(rule.id)!\n\(error.localizedDescription)")
            regex = nil
        }
    ...
    func shouldTriggerFor(request: URLRequest?, response: URLResponse?, error: Error?) -> Bool {
        ...
        // ignore requests to IMQA's back end
        if let endpoints = IMQA.client?.options.endpoints {
