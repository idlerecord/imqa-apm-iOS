//
//  URLSessionCaptureService.swift
//  Imqa-sdk-ios
//
...
//        guard IMQA.client?.config?.isNetworkSpansForwardingEnabled == true else {
//            return false
//        }
//
//        // check local config
    ...
    func install() throws {
        try swizzleClassMethod { originalImplementation -> BlockImplementationType in
            return { urlSession, configuration, delegate, queue -> URLSession in
                let proxiedDelegate = (delegate != nil) ? delegate : IMQADummyURLSessionDelegate()
...
#warning("fix me please in future")
    ...
    func install() throws {
        try swizzleInstanceMethod { originalImplementation -> BlockImplementationType in
            return { [weak handler = self.handler] urlSession, urlRequest -> URLSessionDataTask in
                let request = urlRequest.addIMQAHeaders()
